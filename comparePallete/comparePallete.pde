...
int startX,startY;  // Used to draw pallete
float globalDist;
int[][] pallete = new int[NUMCOLORS][3];
int[][] pallete2 = new int[NUMCOLORS][3];
int [][] currentPallete = new int[NUMCOLORS][3]; // Used for debug
PImage img,img2,img3,img4;
boolean displayPallete = false;
...
void compareWrapper(String filename1, String filename2){
  ...
  img3 = loadImage(filename2);
  size(img.width,img.height);
  int[][] imgData = new int[img.width*img.height][3];
  int[][] img2Data = imgData;
  int[][] img3Data = new int[img3.width*img3.height][3];
  int[][] img4Data = img3Data;
  imgData = getimgData(img);
  pallete = initPallete(imgData);
  img2Data = getPallete(imgData,0);
  printData(pallete);
  img2 = applyKmeans(img2Data,img);
  img3Data = getimgData(img3);
  pallete2 = initPallete(img3Data);
  img4Data = getPallete(img3Data,1);
  printData(pallete2);
  img4 = applyKmeans(img4Data,img3);
  comparePallete(pallete,pallete2);
  image(img,0,0);
}
...
//  }
  int index = 0;
  int median = img.length/2;
  int newMedian;
  int [] targets = new int [NUMCOLORS];
  int [][] pallete = new int[NUMCOLORS][3];
  targets[index] = median;
  index ++;
  for(int i = 1; i < NUMCOLORS/2; i ++){
    newMedian = median/2;
    targets[index] = median + newMedian;
    index ++;
    targets[index] = median - newMedian;
    median = newMedian;
    index ++;
  ...
  for(int i = 0; i < targets.length; i ++){
    pallete[index][0] = img[targets[index]][0];
    pallete[index][1] = img[targets[index]][1];
    pallete[index][2] = img[targets[index]][2];
    index ++;
  }
  return pallete;
}
...
PImage applyKmeans(int[][] pallete,PImage img){
...
int[][] getPallete(int[][] imgData, int img){
  ...
  int [][] imgPallete = new int [NUMCOLORS][3];
  if(img == 1){  // Apply the pallete to the pallete2 global variable for image 2, otherwise the pallete goes 
    imgPallete = pallete2; // to global variable pallete
  } 
  else if(img == 0){
    imgPallete = pallete;
  }
  ...
  while(convergence != true && iterations != ITERATIONS){
    oldPallete = imgPallete;  // store for comparison later
    for(int i = 0; i < imgData.length; i ++){ // For each pixel in the image
      prevDist = getDist(imgData[i],imgPallete[0]); // Initialize comparing distances
      for(int p = 1; p < imgPallete.length; p ++){ // For each pallete color
        distance = getDist(imgData[i],imgPallete[p]);
        if(distance < prevDist){ // The the current pallete color is better...
          matches[i][0] = imgPallete[p][0];   // make it the current match
          matches[i][1] = imgPallete[p][1];
          matches[i][2] = imgPallete[p][2];
          prevDist = distance;
        }
      }
    }
    ...
    for(int p = 0; p < imgPallete.length; p++){  // for each pallete color
      for(int i = 0; i < imgData.length; i ++){ // For each pixel
       totalRed = 0;
       totalGreen = 0;
       totalBlue = 0;
       numMatches = 0;
       if(matches[i][0] == imgPallete[p][0] && matches[i][1] == imgPallete[p][1] && 
         matches[i][2] == imgPallete[p][2]){  // If the pixel matched pallete p
           ...
           numMatches ++;
           kmeansData[i][0] = imgPallete[p][0];  // Apply the pallete to the new image
           kmeansData[i][1] = imgPallete[p][1];
           kmeansData[i][2] = imgPallete[p][2];
         }
       if(numMatches != 0){  // Possible the pallete did not match any color in the image
        ...
        imgPallete[p][0] = totalRed;   // Assign the existing pallete the new r,g,b pallete color
        imgPallete[p][1] = totalGreen;
        imgPallete[p][2] = totalBlue;
        // By now we have a new pallete color at location p in the pallete variable
       }
      }
    }
    convergence = palleteConverge(oldPallete,imgPallete);  // compare the pallete we just calculated with old pallete
  ...
  if(img == 1){
    pallete2 = imgPallete;  // Reassign pallete2 if needed
  }
  else if(img == 0){
    pallete = imgPallete;
  }
